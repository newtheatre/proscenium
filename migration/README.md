...
- **Money** (K-114, I-109): six years of ticket revenue as opening ledger history, from `tickets`,
  ...
  unset on every imported line: `transform-money.ts` does not read `out/performance-id-map.tsv`,
...
- **Programme** (K-113): venues, seasons, show categories, shows, the content-warning vocabulary
  ...
  `performance-id-map.tsv` each map an old id to the unified one, read back before minting so a
  ...
  the reservations transform** (#840), which reads `out/performance-map.tsv` and
  `out/ticket-type-map.tsv` and cannot run without them.
