#!/usr/bin/env bun
// The old rooms app's booking history alone (C-118); build.ts runs the same step in sequence.
import { Database } from 'bun:sqlite'
import { assertLocalTarget, assertNotProduction } from '../tests/helpers/seed'
import { latestStamp } from './lib'
import { closeDumps, openDumps, stepBookings } from './steps'
...
const { summary, exceptions, problems } = await stepBookings(dumps, target)
