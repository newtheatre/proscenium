import { declineAccessProfileForm } from '#shared/utils/access-profiles'
...
export default defineEventHandler(async (event) => {
  ...
  const input = await readValidatedBodyOrThrow(event, declineAccessProfileForm)

  await declineAccessProfile(event, userId, resolved.account.id, input.reason)
