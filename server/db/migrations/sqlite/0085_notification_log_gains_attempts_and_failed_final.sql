...
CREATE TABLE `__new_notification_log` (
	...
	CONSTRAINT "notification_log_status" CHECK("__new_notification_log"."status" IN ('PENDING', 'SENT', 'FAILED', 'RETRYING', 'FAILED_FINAL', 'SUPPRESSED_PREFERENCE', 'SKIPPED_UNDELIVERABLE')),
	CONSTRAINT "notification_log_channel" CHECK("__new_notification_log"."channel" IN ('EMAIL', 'INBOX', 'PUSH'))
);
--> statement-breakpoint
INSERT INTO `__new_notification_log`("id", "user_id", "type", "channel", "subject", "status", "record_id", "session_id", "claim", "sent_at", "error", "attempts", "retry_payload", "created_at") SELECT "id", "user_id", "type", "channel", "subject", "status", "record_id", "session_id", "claim", "sent_at", "error", 0, NULL, "created_at" FROM `notification_log`;--> statement-breakpoint
