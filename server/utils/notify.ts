...
interface Recorded {
  ...
  // (0058). Anything terminal passes nothing and so clears it.
  payload?: string | null
}
...
// from at the next attempt. Cleared with the rest of the payload the moment it settles (0058).
...
export async function resend(event: H3Event | undefined, id: string, maxAttempts: number): Promise<Status> {
  ...
  // judged on the address it carries and nothing else (E-124, 0058).
...
export async function notify(event: H3Event | undefined, notification: Notification): Promise<Status> {
  ...
  try {
    ...
    // The rendered message rides on the row so the sweep can send exactly this again (0058).
    await record({
      ...logged,
      status: 'FAILED',
      subject: rendered.subject,
