...
# `0004_chilly_nighthawk` and `wrangler d1 migrations apply` records
# `0004_chilly_nighthawk.sql`. The health endpoint folds them the same way, in
# shared/utils/migrations.ts.
#
# Exit codes: 0 = ledger read (output may be empty), 2 = could not read it.
set -euo pipefail

DB_NAME="${D1_DATABASE_NAME:-unified}"
