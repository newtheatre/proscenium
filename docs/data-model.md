...
| `GET /api/admin/bar/products` | The paged envelope, every status included, each row carrying whether it has ever sold and `restrictedPours`, the names of the age-restricted stocked items its live sizes pour. Filtered by its declaration (`shared/utils/bar-products-list.ts`, K-129): `categoryId`, `retired` and `withoutCheckId` (an unretired product left unrestricted that pours restricted stock, the Bar Manager's correction list, issue 1299), with `search` over the name and `sort` by category, category name, till order within the category, or product name. |
