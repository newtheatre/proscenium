...
to report until D-126 exists (`docs/known-issues.md`). It also carries the show's guidance, and so
does the confirmation email: `showGuidance()` in `server/utils/reservations.ts` reads the age
guidance and the warnings from the rows the show page reads, and `saysShowGuidance()` words them
the same way the booking form does (D-102 criterion 4, issue 1330). `POST /api/reservations/resend`
...
pours: `stock_movements_sale_exceeds_on_hand` for a `SALE` and
`stock_movements_comp_exceeds_on_hand` for a `COMP`, which depletes exactly as a paid sale would
