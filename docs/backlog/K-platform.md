...
- Acceptance criteria:
  ...
  5. When a screen asks again before an earlier load has answered, only the newest request's
     answer or failure reaches the screen and the device: an older answer arriving late is
     dropped, so a read taken before a sale can never replace one taken after it (#1289).
