...
- Acceptance criteria:
  ...
  7. Added 24 September 2026 (issue 1267, 0091): the sync fetches `https://www.gov.uk/bank-holidays.json` with a ten second timeout, validates it, and replaces the list only on a valid response, keeping stored dates older than the feed's first. A failure leaves the list untouched and is recorded, with no response text, as one word from a fixed vocabulary.
  8. Added 24 September 2026 (issue 1267, 0091): a failed or stale sync is visible. `/api/health` reports the sync as synced, failed, stale (no success for eight days) or never, beside the coverage it already reports; the settings card shows the list read-only with the last successful sync, any failure, and a Sync now action.
  9. Added 24 September 2026 (issue 1267, 0091): a sync still failing six days after its streak began tells the IT Manager through the notification centre, once per person per streak.
- Source: the Theatre Manager's account of the Students' Union's form; decisions 0038 and 0091.
