...
- Acceptance criteria:
  ...
  3. Editing a template affects only performances stamped afterwards; rotas already stamped are untouched. Amended 26 September 2026 (issue 1319): saving a venue template also stamps, in the same transaction, every performance at that venue from tonight's 04:00 that holds no live shift; one already holding a live shift is left to the backfill.
  4. A performance created in a venue with no template stamps zero slots and immediately appears in the unstaffed escalation (E-108) rather than failing silently. Amended 26 September 2026 (issue 1319): this applies to a venue we run. An external venue is template-less by rule (criterion 5) and staffed ad hoc, so its night with no shifts reads "Not rostered" on the board and is left out of the digest.
...
- Acceptance criteria:
  1. `/tonight` is the night's destinations, one tile each, in the order they are tapped on a night (the door, for tickets and passes alike, the till where the viewer's own authority resolves a bar role, tonight at a glance, the checklist, the Challenge 25 register, backstage, contacts and incidents, and emergency last and red), under a show-night header naming the show, the curtain, the venue and who is on shift, and three tiles carrying the house in the words every show-night screen uses for it: sold against capacity, in (admitted through the door) and seats left. The glance is one tap away at `/tonight/glance` and carries the rest: the numbers with the collected-or-expected progress, pass pressure, show info (running time and interval calls, latecomer policy, age guidance and content warnings) and access tonight. The six-action wording this criterion opened with, and the reserved, collected and walk-up headroom it named, were amended in the pull request that put every destination on the hub and settled one vocabulary for the house numbers (issue 1150 items 3 and 11). Trimmed 26 September 2026 to one door tile, where scanning a ticket and admitting a pass holder were two (issue 1301).
  2. Tonight's team is listed by role on the contacts and incidents screen, one tap from the hub, with one-tap contact where the member has consented to shift-visible contact details (A-114); an unfilled slot shows as unfilled, never as a blank name, and the same person covering two of tonight's performances is one row. `GET /api/tonight/team` answers any of tonight's three roles, not the duty manager alone, since the door and the bar are the ones reaching for a number.
  ...
  4. Everything the night needs is at most two taps from the hub: the incident log, the backstage board, the checklist and close-night, and the Challenge 25 register are each one tap from it, and near-miss reporting is one tap on from the incident log. The checklist and the register reached through another screen until issue 1150 item 3 gave each its own tile.
...
- Acceptance criteria:
  ...
  4. The flow is reachable inline from the till when an age-restricted item is sold, and standalone from the tonight screen. Amended 26 September 2026 (issue 1301): the door screen no longer carries its own Challenge 25 button, since the hub's tile is one tap away and the door is one field.
...
- Acceptance criteria:
  ...
  2. The tonight view lists every performance inside the 04:00 boundary in running order and makes the active one unmistakable; switching between them is one tap, and every scan, admit and register entry lands against the performance selected at that moment. Every show-night picker names a performance by its show title and curtain time rather than its database id, and a screen opening cold starts on the active one: `GET /api/tonight/authority` answers with `performances: { id, showTitle, startsAt, venueName, active }[]`, where `active` is `activePerformanceId()`'s own answer, so no screen derives "which house" a second way (issue 901).
