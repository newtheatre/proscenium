...
| **External bookable venues are not built.** C-101 criterion 3 wants campus, building, room and contact details for the SU rooms the society arranges externally, and refusal of deletion while a booking references one. | The estate a member can book is internal rooms only, so an externally arranged venue has nowhere to live and no `AWAITING_EXTERNAL` status exists for it. C-118 expects to import bookings in that status. | C-spaces open question 3, which asks the committee whether externally arranged SU venues stay in scope at all. Building the table before that is answered would be building something that may be dropped. |
| **The booking tier CHECK and the configurable tier list disagree.** `docs/data-model.md` gives `room_bookings.tier` a CHECK of `PRODUCTION\|TRAINING\|REHEARSAL\|GENERAL`; `ROOM_PRIORITY_TIERS` ships `PRODUCTION`, `COMMITTEE`, `REHEARSAL`, `GENERAL`. | A committee-editable list behind a fixed constraint is a setting that breaks writes when it is used. The lists also simply differ on their second value. | C-115, which builds tiers and bumping, and must settle whether the CHECK exists at all (the reasoning in 0033 applies unchanged, `room_bookings` not being append-only). |
