...
- **Sizes are real, and the API derives the rest.** `containerMl` is millilitres in one container
  (700 for a 70 cl bottle); omit it and the product is counted in whole items. `depletesQty` is
  what one sale takes in the *stock* product's basis, so a single is `25`; omit it and a measure
  takes one whole container of its target (ADR-0035).
...
- **`stockOnly` products carry no price and reach no till.** `pricePence` is required unless
  `stockOnly` is set and refused when it is; a `stockProductId` alongside it is refused too,
  because something stock-only holds its own stock. `GET /api/bar/tonight`, the tab menu and the
...
- **GP scales cost by depletion.** A 175 ml glass costs 175/750ths of its bottle's latest delivery
  cost, not a whole one (`docs/13` §3.1). A product with no delivery recorded has a null cost
...
- **`POST /api/admin/bar/stock/adjust`** takes `qtyContainers`, refuses an adjustment aimed at a
  measure rather than the product that holds the stock, and requires a reason.
