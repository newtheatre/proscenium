...
| `*/10 * * * *` | `health:watch` | Opens a `health_incidents` row on the first unhealthy `/api/health` check, notifies the IT Manager through the notification centre once `HEALTH_ALERT_WINDOW_MINUTES` has passed with it still open, and closes it the moment a check recovers so the next failure alerts again from cold (J-106 criterion 5). Bot Fight Mode challenges GitHub runners, so `migrate.yml` no longer checks health (issue 1014); after a migration the check is by hand in a browser, and `.github/workflows/health-watch.yml` is blocked the same way (`docs/operations.md`). |
