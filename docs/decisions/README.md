...
| 0082 | A multi-size product opens a size sheet from one tile, rather than showing its sizes inside the tile | why a pill per size does not fit a 360 pixel card, what the tile says instead so a second tap is never a surprise, and why neither a full-width tile nor a stored default size was taken |
