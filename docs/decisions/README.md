...
| 0098 | An officer's bypass is recorded when it acts, not when it looks | why a role check a screen makes on mount wrote bypass rows for every role an officer held, why a read now records nothing while every write still records once per night, venue and role, why the glance's access wording is the one read that records, and why the night report now lists every role an officer stood in for and whether a confirmed shift existed. Amends 0044 |
