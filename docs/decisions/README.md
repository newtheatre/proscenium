...
| 0091 | Bank holidays are synced from gov.uk and never typed, and a failed sync warns | why the list 0038 left to the committee is now copied weekly from gov.uk, the limits on the Worker's first outbound call, why the write carries no actor, why the sync state is read from the audit trail rather than configuration, and how a failure is reported and escalated. Proposed; direction decided 24 September 2026 by the IT Manager |
