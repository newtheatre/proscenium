...
| 0099 | Tonight's confirmed bar shift may enter stocktake counts | why counting the whole register was one person's phone job, why tonight's confirmed bar shift inside its window may enter and change counts while a stocktake is open, why each line records who counted it, and why opening and applying stay with `bar.write`. Extends 0009. Proposed |
