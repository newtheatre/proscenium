...
| 0091 | Training is recorded by address on a shadow account, as a pending grant is | why a sign-off or certificate may name an address when the picker finds nobody, why it follows 0088 rather than a pending table, why making the account is the narrow `training.by-address` (with department leads derived) rather than `accounts.create`, and why nothing is sent. Accepted 24 September 2026 by the IT Manager |
| 0092 | Bank holidays are synced from gov.uk and never typed, and a failed sync warns | why the list 0038 left to the committee is now copied weekly from gov.uk, the limits on the Worker's first outbound call, why the write carries no actor, why the sync state is read from the audit trail rather than configuration, and how a failure is reported and escalated. Accepted 24 September 2026 by the IT Manager |
...
| 0094 | Tonight is offered wherever a person who can work tonight looks | why three screens answering "on shift tonight" three ways became one derived fact read by the session, why it is a confirmed shift inside its own window (a bar opening included) and never a claim, why a night permission can work tonight without being on shift, why sign-in lands on Tonight only when no `next` is given, and why My rota keeps tonight's shift until 04:00 with the duty manager in place of Release. Amends 0040 |
| 0096 | Every till charge is an attempt, answered by SumUp or by the person at the reader | why the typed path now writes an attempt and records nothing until "Reader took it", why "Card declined" writes nothing rather than reversing a sale, why `POST /api/till/sale` keeps only the tab and the nothing-to-take basket, why a phone with the hand-off shows one charge button, and why the new kind column carries no CHECK. Supersedes 0069 in part |
| 0102 | Paid refunds are approved by the Front of House Manager's own role, on any day | why `FOH_MANAGER` holds `money.refund`, why the duty-manager branch of refund approval goes (no shift reaches the desk), why `REFUND_PAID_REQUIRES_MANAGER` stays though it now separates nobody, and why a request queue waits for V2. Amends 0090. Accepted 26 September 2026 by the IT Manager |
