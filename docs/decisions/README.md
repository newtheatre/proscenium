...
| 0063 | How this repository rebuilds a table that `check:migrations` refuses | why append-only status is not what governs a rebuild, the hand-authored `__new_x` procedure, and why a resolving subquery must leave the copying `SELECT` list |
