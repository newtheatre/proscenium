...
| Second-factor required roles | none: no role required one | ADMIN, MANAGER, THEATRE_MANAGER, TRAINING_MANAGER, ACCESSIBILITY_OFFICER, TREASURER, BOX_OFFICE, BAR_MANAGER, FOH_MANAGER | `PRIVILEGED_ROLES` (A-112). The manual audit of 11 September 2026 found the treasurer reading the whole ledger with a password alone; the proposed default adds every role the key's own description names (money, personal data or safety records), confirmed by a test against `shared/utils/roles.ts`'s permission map. Owner: IT Manager |
