...
| SumUp hand-off timeout | none: the figure was always keyed by hand | 120 minutes | F-124. How long a hand-off to the SumUp app with no answer waits before the sweep abandons it; shipped as this row's proposed value so the till works from day one. A stuck completion is marked mismatched after ten minutes, a technical window rather than a policy one |
...
| Session edit window | 14 days | 14 days | Now enforced: past it a marked register can only be corrected by the IT Manager revoking the record and granting it again (G-114) |
...
| Shift eligibility mapping | none: the old estate faked this with a 45-second cached call to stage-door and admitted everyone on failure | duty manager `ADMN-201` Committee Operations and Governance; door `ADMN-103` Box Office and Ticketing; bar `ADMN-102` Selling Alcohol | Named by the IT Manager on 26 September 2026 (issue 1318), to be confirmed in session, and needed before the rota opens for claims on 5 October. Three settings, one per shift role, each naming the training module that gates it: the duty manager is held by committee members only, the door needs the training on tickets and the bar the training on alcohol sales. Selling Alcohol keeps Front of House Management as its prerequisite, the skills ticketing and the bar share, and is not limited to the committee. A key set to nothing, or naming a module that is still a draft, retired or missing, refuses every claim for its role rather than admitting everyone (E-103 criterion 4); the readiness card on **Shift templates** and `/api/health` both say so. All three modules must be published (ACTIVE) in the live catalogue before 5 October; the card and `/api/health` name any that is not. Owner: Training Manager, with the Front of House Manager |
| Duty manager training chain | the duty manager module needs Committee & Governance first, as the catalogue was reported configured on 23 September 2026 | withdraw Committee & Governance from the duty manager chain; make it a committee-only module worth 12 months from award | Raised 24 September 2026 (issue #1263, module E open question 2). A catalogue and settings change made in the console (the module's **Needs first** and **How long it is worth**, and `SHIFT_ELIGIBILITY_DUTY_MANAGER_MODULE` if the gating module changes), not code. The catalogue cannot restrict sign-up to committee members; committee-only means only the committee's sessions teach it. Settled 26 September 2026 (issue 1318): the duty manager shift is gated on Committee Operations and Governance (`ADMN-201`) itself, so it is held by committee members only. Owner: Training Manager, with the committee |
| Shift claim auto-confirm | the old estate had the toggle; no recorded default | auto-confirm | Module E open question 1. Eligibility is already re-checked live against training records at the claim itself (E-103, E-104), so queuing adds a manual step without adding safety; a committee that wants tighter control can switch to queue mode as a settings change, not a release. In queue mode only `rota.write` holders (the FOH officer and administrators) confirm or decline, the same audience that administers shift templates. Owner: IT Manager |
| Shift window around a performance | none: a shift had no times at all, and authority ran the whole 04:00 night | 30 minutes before doors, 30 minutes after the end | E-131, decision 0078. Two settings, `SHIFT_START_BEFORE_DOORS_MINUTES` and `SHIFT_END_AFTER_CURTAIN_DOWN_MINUTES`, stamped onto a shift when it is stamped. The end counts from curtain plus running time plus intervals. A venue's template overrides either offset per role, so these are the house default rather than the only answer, and a bar that wants an hour either side sets it on its own template. Owner: IT Manager |
| Shift authority grace | none: authority held for the whole show night | 30 minutes either side | E-131, decision 0078. `SHIFT_AUTHORITY_GRACE_MINUTES`. How far outside their own window a confirmed shift still opens its tool, so an early arrival or a late close-down is not refused by the clock. The 04:00 boundary still bounds everything whatever this says, and the officer bypass has no window. Owner: IT Manager |
| Shift release notice window | none | 48 hours | E-107 criterion 2. A release inside this window of the performance notifies the FOH officer immediately; an earlier one only appears in E-108's seven-day digest. A released duty-manager shift always notifies immediately, whatever the window says, because the night cannot legally run without one. Owner: IT Manager |
...
| Where to buy a membership | nowhere: no page or email named the SU's purchase flow | none; ships unset | A-202, issue 1005. `MEMBERSHIP_PURCHASE_URL`, the address of the SU's own page for buying a New Theatre membership. **The committee is asked for it**, and whether it is stable enough to live in a setting. Until it is set, the membership page, the policy page, the expiring and declined emails and the room booking refusal say "bought at the Students' Union" with no link, rather than a guessed address. Must be `https://`. Owner: IT Manager |
...
| Role lapse notice | none: a role lapsed without warning | 14 days before the grant expires | A-119, whose story asks for two weeks. Shipped as this row's proposed value rather than left unset, so the warning is live rather than a dormant feature awaiting a number; this row is what confirms it, not the code default. Owner: IT Manager |
| Re-authentication freshness | none: a security-settings screen forced a full sign-out and sign-in | 10 minutes | A-128. The window a proven credential stays fresh enough to change a security setting or re-assert for a sensitive action, carrying forward the figure the security screen already used unconfigured. Owner: IT Manager |
| Lapsed grant tidying | none: lapsed rows stayed forever | delete 90 days after the grant expired | A-119 criterion 4. Housekeeping rather than a policy number: enforcement is read-time, so the row granted nothing from the instant it expired, and the audit trail keeps what it said. Owner: IT Manager |
| Retention periods | 2 years full accounts, 3 years guests (dry-run) | unchanged, armed in December | |
| Send-log retention | rehearsal pruned its notification ledger at 24 months; the other three apps kept everything | 24 months | New setting (H-105 criterion 5), and the answer to module H's open question 3. Matching rehearsal because it is the only period the estate has ever operated, and the log feeds a subject access request, so this is a retention decision rather than a tidy-up. Owner: IT Manager |
| Send retries | none: a failed send was lost in three apps and retried by cron in one | 5 attempts, first retry after 10 minutes, doubling | New settings (H-105 criterion 2). Five attempts at a doubling ten minutes span about two and a half hours, which covers a provider outage without a message arriving so late it is stale. Owner: IT Manager |
| Digest coalescing window | none: rehearsal's digests ran on their own fixed schedules, and no other app coalesced at all | 60 minutes, per topic | New settings (H-104 criterion 2, and the answer to module H's open question 2): one scalar key per topic rather than one shared value, so a workshop can shorten shifts or room bookings without a release; all five ship at the same figure until one is asked to. Owner: IT Manager |
...
| Second-factor required roles | none: no role required one | ADMIN, MANAGER, THEATRE_MANAGER, TRAINING_MANAGER, ACCESSIBILITY_OFFICER, TREASURER, BAR_MANAGER, FOH_MANAGER, SAFETY_OFFICER | `PRIVILEGED_ROLES` (A-112). The manual audit of 11 September 2026 found the treasurer reading the whole ledger with a password alone; the proposed default adds every role the key's own description names (money, personal data or safety records), confirmed by a test against `shared/utils/roles.ts`'s permission map. The role audit on #1211 found the Safety Officer, who holds safety records, still outside it; the IT Manager added it on 23 September 2026. Owner: IT Manager |
| Role vocabulary mapping | four namespaces | one officer model, mapping table agreed in session | Needed by the Phase 1 import. Settled already: the old box office roles map to `FOH_MANAGER`, the Box Office Manager and the Front of House Manager being one post (0090) |
...
proposed value: the night report recipients, which ships unset until a session confirms it
(0019). Retention is the reverse case: its periods and caps carry values, but no session has
...
emailing every ticket holder on its own.
