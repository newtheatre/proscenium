...
const { data, status, error, refresh } = await useAsyncData(
  'seasons',
  () => request<Listing>('/api/admin/reference-data/seasons', {
      ...
      <template #filters>
        <UFormField label="Show">
          <USwitch
            ...
            label="Including retired seasons"
            data-test="seasons-archived"
          />
        </UFormField>
      </template>
    ...
    >
      <template #empty>
        <p class="py-6 text-center text-sm text-muted">
          {{ search ? 'No season matches that.' : 'No seasons yet. Add one, and a show has something to belong to.' }}
        </p>
      </template>
    </UTable>
