...
describe.skipIf(skip !== null)('opening opens practice windows (G-115 criterion 3)', () => {
  test('one window per placed member per matching target, and never two', async () => {
    const module = await addModule()
    const key = `sandbox-${suffix().toLowerCase()}`
    expect((await send('POST', '/api/admin/training/practice-targets', {
      key,
      name: 'A sandbox',
      windowHours: 72,
      moduleIds: [module],
    ...
    expect((await openRegister(session)).status).toBe(200)
    expect(read<{ n: number }>(
      'SELECT count(*) n FROM practice_windows WHERE session_id = ? AND target_key = ?', session, key,
    )?.n).toBe(1)

    // Criterion 4: opening again opens no second window.
    await openRegister(session)
    expect(read<{ n: number }>(
      'SELECT count(*) n FROM practice_windows WHERE session_id = ?', session,
    )?.n).toBe(1)
  })

  test('a retired target opens nothing', async () => {
    const module = await addModule()
    const key = `retired-${suffix().toLowerCase()}`
    await send('POST', '/api/admin/training/practice-targets', {
      key,
      name: 'Out of use',
      windowHours: 72,
      isActive: false,
