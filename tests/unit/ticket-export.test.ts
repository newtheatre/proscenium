...
import { resolveYearBounds } from '#shared/utils/ticket-export'
import { londonParts } from '#shared/utils/london'

// D-129 criterion 5: the year boundary a report resolves to, proved against the calendar
// rather than against a raw epoch nobody can eyeball.

describe('resolveYearBounds runs 1 August to 31 July, London (D-129 criterion 5)', () => {
  test('the default boundary opens on 1 August of the year before', () => {
    const { fromAt } = resolveYearBounds(2027, '08-01', '07-31')
  ...
  test('the boundary closes at the last instant of 31 July of the named year, exclusive', () => {
    const { toAt } = resolveYearBounds(2027, '08-01', '07-31')
  ...
  test('the boundary carries the DST shift the calendar has in August and none in the following July', () => {
    ...
    const { fromAt, toAt } = resolveYearBounds(2027, '08-01', '07-31')
  ...
  test('a configured boundary other than the default resolves the same way', () => {
    const { fromAt, toAt } = resolveYearBounds(2027, '09-01', '08-31')
  ...
  test('successive years do not overlap and leave no gap', () => {
    const first = resolveYearBounds(2027, '08-01', '07-31')
    const second = resolveYearBounds(2028, '08-01', '07-31')
