...
export const SEASONS_OFFERED = 6

export interface Choice<T> { label: string, value: T }

// The two years a season spans. The second is shortened only where the century does not turn,
// so 2099/2100 still reads as two years rather than as 2099/00.
export function saysSeasonYear(year: number): string {
  const began = year - 1
...
// Newest first: a treasurer means this season unless they say otherwise (K-123 criterion 2).
export function seasonChoices(latest: number, count = SEASONS_OFFERED): Choice<number>[] {
  return Array.from({ length: count }, (_, back) => ({
    label: saysSeasonYear(latest - back),
    value: latest - back,
  }))
}

// A month's year is a calendar year, not a season, so it is offered as one: "2026", not
// "2025/26". Newest first, the same depth as the seasons.
export function yearChoices(latest: number, count = SEASONS_OFFERED): Choice<number>[] {
